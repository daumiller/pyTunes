...
# ==============================================================================
def library_update(db, path):
    global INDEX_ARTIST
    global INDEX_ALBUM
    global INDEX_SONG
    INDEX_SONG   = int(database_execute(db, 'SELECT indexSong FROM song ORDER BY indexSong DESC')[0][0]) + 1
    INDEX_ALBUM  = int(database_execute(db, 'SELECT indexAlbum FROM album ORDER BY indexAlbum DESC')[0][0]) + 1
    INDEX_ARTIST = int(database_execute(db, 'SELECT indexArtist FROM artist ORDER BY indexArtist DESC')[0][0]) + 1
...
def library_scan_file(db, path):
    global PATH_ROOT
    global PATH_SERVED
    db_path = ''.join([PATH_SERVED, path[len(PATH_ROOT):]])
    lookup = database_execute(db, 'SELECT indexSong FROM song WHERE (file = ?)', [db_path])
    if len(lookup): return  # already in our db
...
db = database_connect()
library_update(db, PATH_ROOT)
elapsed = int(time.monotonic() - began)
sys.stdout.write('\b' * 80)
print("Processed {} artists, {} albums, {} songs in {} seconds.".format(NEW_ARTISTS, NEW_ALBUMS, NEW_SONGS, elapsed))
